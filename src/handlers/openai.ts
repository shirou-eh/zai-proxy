...
export async function handleOpenAIChat(
  ...
  const includeUsage = Boolean((body as unknown as { stream_options?: { include_usage?: boolean } }).stream_options?.include_usage);

  const backendBody: Record<string, unknown> = { ...(normalized as unknown as Record<string, unknown>) };
  delete backendBody['stream_options'];

  if (isStream) await handleOpenAIStream(backendBody, clientModel, backendModel, includeUsage, res, config, logger, requestId);
  else await handleOpenAINonStream(backendBody, clientModel, backendModel, res, config, logger, requestId);
}
...
async function handleOpenAIStream(
  ...
  logger.debug(requestId, `OPENAI STREAM start model=${backendModel} client=${clientModel}`);
  ...
  // Heartbeat каждые 15с чтобы держать соединение (некоторые прокси отваливаются)
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (!clientClosed) {
        // Отдаём ошибку как SSE error event (совместимо с OpenAI SDK)
    ...
    if (!r.body) {
      if (!clientClosed) {
        res.write(sseEncode({ error: { message: 'backend returned no body', code: 502 } }));
        res.end(sseDone());
      }
      return;
    }
    ...
    while (true) {
      ...
      for (const raw of lines) {
        ...
        if ((chunk as { error?: unknown }).error !== undefined) {
          // Пробрасываем error напрямую
          if (!clientClosed) res.write(`data: ${JSON.stringify(p)}\n\n`);
          continue;
        }

        if (chunk.choices !== undefined) {
          for (const c of chunk.choices) if (c.finish_reason) lastFinish = c.finish_reason as string;
        ...
        } else if ((chunk as unknown as { usage?: unknown }).usage && includeUsage) {
          if (!clientClosed) res.write(sseEncode({ choices: [], usage: (chunk as unknown as { usage: unknown }).usage }));
        } else {
          const maybeUsage = (p as Record<string, unknown>)['usage'];
          if (maybeUsage && includeUsage && !clientClosed) res.write(sseEncode({ choices: [], usage: maybeUsage }));
        }
      }
    }
    ...
    logger.debug(requestId, `OPENAI STREAM done backend=${backendModel} done=${done}`);
  } catch (e) {
...
async function handleOpenAINonStream(
  ...
  try {
    ...
    if (!r.ok) {
      ...
      // Пробуем отдать оригинальный JSON ошибки если он уже в формате {error:{message}}
      try {
